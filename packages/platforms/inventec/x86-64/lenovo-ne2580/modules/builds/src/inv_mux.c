...
int
cpld_rst_all_4_pull_low_1(struct mux_obj_s *self){
    ...
    if(is_unexpected){
        emsg = "Undefined case";
        goto err_cpld_rst_all_4_pull_low_1;
    }
...
int
cpld_rst_all_4_pull_low_2(struct mux_obj_s *self){
    ...
    switch(self->gpio_num) {
        case MUX_RST_CPLD_C0_A77_70_RST_ALL:
            is_unexpected = 0;
            break;
    ...
    if(is_unexpected){
        emsg = "Undefined case";
        goto err_cpld_rst_all_4_pull_low_2;
    }
...
int
cpld_rst_all_4_pull_high_1(struct mux_obj_s *self){
    ...
    if(is_unexpected){
        emsg = "Undefined case";
        goto err_cpld_rst_all_4_pull_high_1;
    }
...
err_cpld_rst_all_4_pull_high_1:
    SWPS_INFO("%s: %s <type>:%d <err>:%d\n",
            __func__, emsg, self->gpio_num, err);
    return ERR_MUX_UNEXCPT;
}
...
int
cpld_rst_all_4_pull_high_2(struct mux_obj_s *self){
    ...
    if(is_unexpected){
        emsg = "Undefined case";
        goto err_cpld_rst_all_4_pull_high_2;
    }

    err = _setup_i2c_value(self, 0x70, 0xff);
    if (err < 0) {
        emsg = "setup 0x70 fail";
        goto err_cpld_rst_all_4_pull_high_2;
    }
...
err_cpld_rst_all_4_pull_high_2:
    SWPS_INFO("%s: %s <type>:%d <err>:%d\n",
            __func__, emsg, self->gpio_num, err);
    return ERR_MUX_UNEXCPT;
}
...
_setup_muxctl_cb(struct mux_obj_s *self,
    ...
    switch (gpio) {
        ...
        case MUX_RST_CPLD_C0_A77_70_74_RST_ALL:
            self->gpio_num   = gpio;
            self->_pull_low  = cpld_rst_all_4_pull_low_1;
            self->_pull_high = cpld_rst_all_4_pull_high_1;
        ...
        case MUX_RST_CPLD_C0_A77_70_RST_ALL:
            self->gpio_num   = gpio;
            self->_pull_low  = cpld_rst_all_4_pull_low_2;
            self->_pull_high = cpld_rst_all_4_pull_high_2;
            self->_init      = init_cpld_4_rst_all;
            self->_clean     = clean_cpld_4_rst_all;
            self->reset      = cpld_reset_mux_all;
            memset(mod_dsc, 0, 32);
