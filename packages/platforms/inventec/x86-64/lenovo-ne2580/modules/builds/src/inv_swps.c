...
show_attr_version(struct device *dev_p,
    ...
    return snprintf(buf_p, sizeof(SWP_VERSION)+1, "%s\n", SWP_VERSION);
}
...
get_platform_type(void){
    ...
    int i;
    int sfp_ioexp_dev, cpld2nd_dev;
    unsigned char conclusion = 0;
    int auto_chan = -1;
    int auto_addr = -1;
    ...
    switch (PLATFORM_SETTINGS) {
        ...
        case PLATFORM_TYPE_PEONY_AUTO:
            ...
            auto_chan = peony_sfp1_ioexp_layout[0].addr[0].chan_id;
            auto_addr = peony_sfp1_ioexp_layout[0].addr[0].chip_addr;
#endif
            sfp_ioexp_dev = _is_i2c_target_exist(auto_chan, auto_addr);
            
            auto_chan = 0;
            auto_addr = 0x33; // Addr of 2nd cpld from new model 2.
            cpld2nd_dev = _is_i2c_target_exist(auto_chan, auto_addr);
            conclusion = (sfp_ioexp_dev << 1) | cpld2nd_dev;
            switch (conclusion) {
                case PEONY_COPPER_ONE_CPLD: /* Copper SKU */
                case PEONY_COPPER_TWO_CPLD:
                    SWPS_INFO("Auto-detected <platform>:Peony <SKU>:Copper\n");
                    platform_p->id = PLATFORM_TYPE_PEONY_COPPER_GA;
                    goto map_platform_name;
                case PEONY_SFP_EVT: /* SFP EVT SKU */
                    SWPS_INFO("Auto-detected <platform>:Peony <SKU>:SFP1\n");
                    platform_p->id = PLATFORM_TYPE_PEONY_SFP1_GA;
                    goto map_platform_name;
                case PEONY_SFP_DVT: /* SFP DVT SKU */
                    SWPS_INFO("Auto-detected <platform>:Peony <SKU>:SFP2\n");
                    platform_p->id = PLATFORM_TYPE_PEONY_SFP2_GA;
                    goto map_platform_name;
                default:
                    break;
            }
            snprintf(log_msg, sizeof(log_msg),
                    "Auto detect Peony SKU fail! <err>:%d", sfp_ioexp_dev);
            goto err_get_platform_type_2;

        case PLATFORM_TYPE_PEONY_SFP1_GA:
        case PLATFORM_TYPE_PEONY_SFP2_GA:
...
#ifdef SWPS_PEONY_SFP1
        case PLATFORM_TYPE_PEONY_SFP1_GA:
            gpio_rest_mux = peony_sfp1_gpio_rest_mux;
            ioexp_layout  = peony_sfp1_ioexp_layout;
            port_layout   = peony_sfp1_port_layout;
            ioexp_total   = ARRAY_SIZE(peony_sfp1_ioexp_layout);
            port_total    = ARRAY_SIZE(peony_sfp1_port_layout);
            break;
#endif
#ifdef SWPS_PEONY_SFP2
        case PLATFORM_TYPE_PEONY_SFP2_GA:
            gpio_rest_mux = peony_sfp2_gpio_rest_mux;
            ioexp_layout  = peony_sfp2_ioexp_layout;
            port_layout   = peony_sfp2_port_layout;
            ioexp_total   = ARRAY_SIZE(peony_sfp2_ioexp_layout);
            port_total    = ARRAY_SIZE(peony_sfp2_port_layout);
            break;
#endif
...
register_ioexp_attr(struct device *device_p,
    ...
    switch (transvr_obj->ioexp_obj_p->ioexp_type){
        ...
        case IOEXP_TYPE_QSFP_8P_LAYOUT_1:
