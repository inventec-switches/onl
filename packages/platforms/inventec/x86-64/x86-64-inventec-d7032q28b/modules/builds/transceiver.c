...
common_get_eeprom(struct transvr_obj_s *self, char *buf){
    ...
    *(buf+self->eeprom_map_p->length_eeprom) = '\n';
    ...
    /* output printbale char if it is */
    if (self->state == STATE_TRANSVR_CONNECTED &&
        self->mode == TRANSVR_MODE_POLLING &&
        TRANSVR_INFO_CACHE_ENABLE) {
	memset(buf, 0, self->eeprom_map_p->length_eeprom);
	for (i = 0; i < self->eeprom_map_p->length_eeprom; i++) {
	    if (i%16 == 0) {
		sprintf (buf, "%s\n",buf);
	    }
	    if (isprint(*ep)) {
		status = sprintf (buf, "%s  %c",buf,*ep);
	    }
	    else {
		status = sprintf (buf, "%s %02x",buf,(unsigned char)*ep);
	    }
	    buf+=status;
	    ep++;
	}
	status = sprintf(buf, "%s \n",buf);
	return self->eeprom_map_p->length_eeprom * 3;
    }
    err = _check_by_mode(self, &_common_update_attr_eeprom,
                         "common_get_eeprom");
    if (err < 0){
    ...
    for (i = 0; i < self->eeprom_map_p->length_eeprom; i++) {
        ...
        if (isprint(*ep)) {
            ...
            status = sprintf (buf, "%s %02x",buf,*ep);
        }
	buf+=status;
	ep++;
    }
